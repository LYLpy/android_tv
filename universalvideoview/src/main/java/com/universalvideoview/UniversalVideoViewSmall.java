...
/**
 * 江西广电项目中，自定义的小窗口使用的播放器
 */
public class UniversalVideoViewSmall extends SurfaceView
...
//    所有的mMediaController.show();都关掉了
    ...
    public UniversalVideoViewSmall(Context context) {
        this(context,null);
    }

    public UniversalVideoViewSmall(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public UniversalVideoViewSmall(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mContext = context;
        TypedArray a = mContext.obtainStyledAttributes(attrs, R.styleable.UniversalVideoViewSmall,0,0);
        mFitXY = a.getBoolean(R.styleable.UniversalVideoViewSmall_uvv_fitXY_small, false);
        mAutoRotation = a.getBoolean(R.styleable.UniversalVideoViewSmall_uvv_autoRotation_small, false);
        a.recycle();
        initVideoView();
    }
    private int mWidthMeasureSpe;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //重新计算高度，让视频铺满VideoView
//        int width = getDefaultSize(0, widthMeasureSpec);
//        int height = getDefaultSize(0, heightMeasureSpec);
//        setMeasuredDimension(width, height);
        mWidthMeasureSpe = getDefaultSize(0, widthMeasureSpec);
        mHeightMeasureSpec = getDefaultSize(0, heightMeasureSpec);
        setMeasuredDimension(mWidthMeasureSpe, mHeightMeasureSpec);
//        Log.e("__UVideoView_onMea_w",mWidthMeasureSpe + "__");
//        Log.e("__UVideoView_onMea_h",mHeightMeasureSpec + "__");
    }
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(UniversalVideoViewSmall.class.getName());
    }
    ...
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            info.setClassName(UniversalVideoViewSmall.class.getName());
        }
    }
...
//                                                requestLayout();
                        if (mWidthMeasureSpe != 0 && mHeightMeasureSpec != 0) {
                            //之前出现视频超出VideoView的情况，可以在这里操作，设置视频宽高，避免超出控件
                            ViewGroup.LayoutParams lp = UniversalVideoViewSmall.this.getLayoutParams();
                            lp.width = mWidthMeasureSpe;
                            lp.height = mHeightMeasureSpec;
                            getHolder().setFixedSize(lp.width, lp.height);
                            UniversalVideoViewSmall.this.setLayoutParams(lp);
                        }
                    }
                }
            };
...
//                    }
    ...
    private void enableOrientationDetect() {
        if (mAutoRotation && mOrientationDetector == null) {
            ...
            mOrientationDetector.setOrientationChangeListener(UniversalVideoViewSmall.this);
